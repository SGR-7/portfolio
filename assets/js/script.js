...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const cards = document.querySelectorAll('section[id^="projects-page"] .project-card');
...
loadParticles('particles-education');

/* ===== assets/js/portfolio.js ===== */
/*
 * Portfolio-specific JavaScript.
 * The original Photon scripts remain untouched.
 */

document.addEventListener('DOMContentLoaded', function () {

    // Project cards work on every section whose id starts with "projects-page".
    // Project details are read directly from each card's HTML, so adding a new
    // project/page never requires editing this JavaScript.
    const projectCards = document.querySelectorAll('section[id^="projects-page"] .project-card');
    const canHoverProjects = window.matchMedia('(hover: hover)').matches;
    let activeProjectPanel = null;
    let projectDetailsTimer = null;

    function hideProjectDetails() {
        clearTimeout(projectDetailsTimer);
        projectDetailsTimer = null;

        if (activeProjectPanel) {
            activeProjectPanel.classList.remove('is-visible');
            activeProjectPanel.setAttribute('aria-hidden', 'true');
    ...
    function showProjectDetails(card) {
        ...
        const page = card.closest('section[id^="projects-page"]');
        const panel = page ? page.querySelector('.project-details-panel') : null;
        const content = panel ? panel.querySelector('.project-details-content') : null;
        const details = card.querySelector('.project-card-details');

        if (!panel || !content || !details) return;

        if (activeProjectPanel && activeProjectPanel !== panel) {
            activeProjectPanel.classList.remove('is-visible');
            activeProjectPanel.setAttribute('aria-hidden', 'true');
        }

        activeProjectPanel = panel;
        projectDetailsTimer = setTimeout(function () {
    ...
    const certificatePanel =
        document.querySelector('#education .certificate-details-panel');

    const certificatePreview =
        document.querySelector('#education .certificate-preview-image');

    const canHoverCertificates =
    ...
    // Show the PDF
    function showCertificatePreview(card) {

        clearTimeout(certificateTimer);

        if (!certificatePanel || !certificatePreview) {
            return;
        }

        /*
         * The PDF address is already written in the card's href.
         *
         * Example:
         *
         * href="assets/certificates/my-certificate.pdf"
         *
         * We simply take that address.
         */
        ...
        certificateTimer = setTimeout(function () {

            // Put the certificate PDF inside the center panel
    ...
    // Add the behavior to every certificate card
    certificateCards.forEach(function (card) {

        // Desktop mouse hover
        if (canHoverCertificates) {

            card.addEventListener('mouseenter', function () {
                showCertificatePreview(card);
            });
    ...
    // Add a subtle active state to internal navigation while scrolling.
...
document.getElementById('more-projects-button').addEventListener('click', function (e) {
    e.preventDefault();

    const page = document.getElementById('projects-page2');
...
document.getElementById('less-projects').addEventListener('click', function (e) {
    e.preventDefault();

    const page = document.getElementById('projects-page2');

    $('html, body').animate({
        scrollTop: document.getElementById('projects-page1').offsetTop
    }, 1000, function () {
